...
class MemristorArray(torch.nn.Module):
    ...
    def __init__(
        self,
        mem_device: dict = {},  # device_name, c2c_variation, d2d_variation, fault, retention_loss, aging
        shape: Optional[Iterable[int]] = None,  # size of the crossbar
        memristor_info_dict: dict = {},  # Gon, Goff, Gon_sigma, Goff_sigma
        **kwargs,
    ) -> None:
        # language=rst
        """
        ...
        self.register_buffer(
            "mem_x", torch.Tensor()
        )  # Memristor 0-1 internal state (0 is low conductance)
        self.register_buffer("mem_c", torch.Tensor())  # Memristor conductance
        self.register_buffer("mem_t", torch.Tensor())  # Memristor simulation time
    ...
    def set_batch_size(self, batch_size) -> None:
        ...
        if self.c2c_variation:
            self.normal_relative = torch.zeros(
                batch_size, *self.shape, device=self.normal_relative.device
            )
            self.normal_absolute = torch.zeros(
                batch_size, *self.shape, device=self.normal_absolute.device
            )
        ...
        if self.aging_effect:
            ...
            if self.d2d_variation in [1, 2]:
                ...
                self.Gon_0 = self.memristor_info_dict[self.device_name][
                    "G_on"
                ] * torch.ones(batch_size, *self.shape, device=self.Gon_0.device)
                self.Goff_0 = self.memristor_info_dict[self.device_name][
                    "G_off"
                ] * torch.ones(batch_size, *self.shape, device=self.Goff_0.device)

        if self.stuck_at_fault:
            SAF_lambda = self.memristor_info_dict[self.device_name]["SAF_lambda"]
            SAF_ratio = self.memristor_info_dict[self.device_name][
                "SAF_ratio"
            ]  # SAF0:SAF1
            ...
            self.SAF1_mask = (
                self.Q_mask >= ((SAF_ratio / (SAF_ratio + 1)) * SAF_lambda)
            ) & (self.Q_mask < SAF_lambda)

        if self.retention_loss == 2:
            self.mem_v_threshold = torch.zeros(
                batch_size, *self.shape, device=self.mem_v_threshold.device
            )
            self.mem_loss_time = torch.zeros(
                batch_size, *self.shape, device=self.mem_loss_time.device
            )
    ...
    def memristor_compute(self, mem_v: torch.Tensor, mem_t: torch.Tensor) -> None:
        ...
        if self.d2d_variation in [1, 3]:
            self.mem_x = torch.where(
                mem_v >= v_off,
                self.mem_x
                + delta_t
                * (k_off * (mem_v / v_off - 1) ** alpha_off)
        ...
        if self.retention_loss == 1:
            ...
            self.mem_x *= torch.exp(
                torch.tensor(
                    -((1 / 4 * delta_t * retention_loss_tau) ** retention_loss_beta)
                )
            )
        ...
        if self.retention_loss == 2:
            ...
            self.mem_v_threshold = torch.where(
                (mem_v > v_on) & (mem_v < v_off),
                torch.zeros_like(mem_v),
            ...
            self.mem_x -= (
                ...
                * retention_loss_beta
                * self.mem_loss_time ** (retention_loss_beta - 1)
            )
            self.mem_x = torch.clamp(self.mem_x, min=G_on, max=G_off)
            self.mem_x = (self.mem_x - G_on) / (G_off - G_on)

        if self.c2c_variation:
            self.normal_relative.normal_(mean=0.0, std=sigma_relative)
            self.normal_absolute.normal_(mean=0.0, std=sigma_absolute)

            device_v = (
                torch.mul(self.mem_x, self.normal_relative) + self.normal_absolute
            )
    ...
    def update_SAF_mask(self) -> None:
        if self.stuck_at_fault:
            ...
            if increase_ratio > 0 and SAF_delta > 0:
                self.Q_mask.uniform_()
                self.SAF0_mask += (~(self.SAF0_mask + self.SAF1_mask)) & (
                    self.Q_mask < ((SAF_ratio / (SAF_ratio + 1)) * increase_ratio)
                )
                self.SAF1_mask += (~(self.SAF0_mask + self.SAF1_mask)) & (
                    (self.Q_mask >= ((SAF_ratio / (SAF_ratio + 1)) * increase_ratio))
                    & (self.Q_mask < increase_ratio)
                )
